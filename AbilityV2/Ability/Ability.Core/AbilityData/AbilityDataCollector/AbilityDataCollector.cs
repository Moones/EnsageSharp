...
namespace Ability.Core.AbilityData.AbilityDataCollector
{
    ...
    [Export(typeof(IAbilityDataCollector))]
    internal class AbilityDataCollector : IAbilityDataCollector
    {
        #region Fields
        ...
        /// <summary>Initializes a new instance of the <see cref="AbilityDataCollector" /> class.</summary>
        internal AbilityDataCollector()
        {
            this.menu = new Menu(nameof(AbilityDataCollector), Constants.AssemblyName + nameof(AbilityDataCollector));
            this.updateInterval =
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (var keyValuePair in this.AbilityUnitManager.Value.Units)
            {
                    ...
                    && keyValuePair.Value.SourceUnit.IsControllable)
                {
                    ...
                    {
                        if (valueOrderIssuer.Value.PreciseIssue())
                        {
                            break;
                        }
                    }
        ...
        private void Entity_OnInt32PropertyChange(Entity sender, Int32PropertyChangeEventArgs args)
        {
            ...
            if (sender is Creep || sender is Building)
            {
                return;
            }
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            foreach (var keyValuePair in this.AbilityUnitManager.Value.Units)
            {
                    ...
                    && keyValuePair.Value.SourceUnit.IsControllable)
                {
                    foreach (var valueOrderIssuer in keyValuePair.Value.OrderIssuers)
                    {
                        if (valueOrderIssuer.Value.Issue())
                        {
                            orderCount++;
                            break;
                        }
        ...
        private void ObjectManager_OnAddEntity(EntityEventArgs args)
        {
            DelayAction.Add(
                ...
                () =>
                    {
                        this.AbilityMapDataProvider.Value.EntityAdded(args.Entity);
                        return;
